...
window.onload = async function () {
  ...
  // ---- LAAD CENTRALE DATA ----
  let savedCustomers = await fetchCustomers();
  let checkedCustomers = {}; // Alleen lokaal, niet gedeeld
  ...
  for (const klantDatum of Object.keys(savedCustomers)) {
    if (klantDatum < vandaagStr && savedCustomers[klantDatum] && savedCustomers[klantDatum].length > 0) {
    ...
    (saveCustomerBtn.onclick = async function () {
      ...
      if (name && id && jobTypes.length > 0 && employees.length > 0) {
        ...
        if (!tbody) {
          ...
          await saveCustomers(savedCustomers);
          location.reload();
          return;
        }
  ...
  function createCustomerRow(
    ...
    checkbox.addEventListener("change", function () {
      ...
      // checkedCustomers is alleen lokaal, niet opslaan op server
    });
  ...
  actionDelete &&
    (actionDelete.onclick = async function () {
      if (selectedRow) {
        const klantObj = {
